...
// custom_mutator
struct custom_mutator* load_custom_mutator(const char* fn) {

  #ifdef DEBUG 
  fprintf(stderr, "Found mutator: %s \n", fn);
  ...
  if (mutator->afl_custom_init) {
    ...
    mutator->data = mutator->afl_custom_init(R(0xFFFFFFFF));
...
void destroy_custom_mutators() {

  if (mutator_count) {
    LIST_FOREACH_CLEAR(&mutator_list, struct custom_mutator, {
      ...
      if (el->post_process_buf) {
        if (el->post_process_buf) {
          free(afl_alloc_bufptr(el->post_process_buf));
        }
...
void setup_custom_mutators(char* fn) {
  ...
  if (fn) {
    ...
    if (likely(!fn_token)) {
      ...
      mutator = load_custom_mutator(fn);
      list_append(&mutator_list, mutator);
      mutator_count++;
      ...
      while (fn_token) {
        ...
        if (*fn_token) {  // strsep can be empty if ";;"
          ...
          #endif
          prev_mutator_count = mutator_count;
          mutator = load_custom_mutator(fn_token);
          list_append(&mutator_list, mutator);
          mutator_count++;
          if (prev_mutator_count > mutator_count)
...
int main(int argc, char** argv) {
  ...
  // custom_mutator
  char* env_val = NULL;
  if ((env_val = getenv("AFL_CUSTOM_MUTATOR_LIBRARY"))) {
    if (*env_val) {
      OKF("Enabled Custom Mutator with %s", env_val);
      // custom_mutator
      setup_custom_mutators(env_val);
